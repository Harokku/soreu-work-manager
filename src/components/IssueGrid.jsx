...
function IssueGrid(props) {
    ...
    const categories = createMemo(() => {
        const categories = Object.keys(issueStore.aggregatedData());
        ...
        <>
            <div className="flex flex-wrap gap-6 mx-6">
                <div>
                    ...
                    <Show when={issueStore.aggregatedData().self} fallback={<div>Nessuna nota personale</div>}>
                        <IssueList category={issueStore.aggregatedData().self}/>
                    </Show>
                </div>
                <For each={categories()} fallback={<div>Loading...</div>}>
                    {(category) => (
                        <div>
                            ...
                            <IssueList category={issueStore.aggregatedData()[category]}/>
                        </div>
                    )}
                </For>
            </div>
